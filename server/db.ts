import { defaultXifadoData, type XifadoData } from "../shared/xifado.js";
import { hashParticipantPassword } from "./security.js";
import { supabaseAdmin } from "./supabase.js";
import { getCredentialHash, migrateLegacyCredentialHashes, upsertCredentialHash } from "./credentials.js";
...
async function initialXifadoData(): Promise<XifadoData> {
  ...
  try {
    ...
    for (const [name, password] of Object.entries(credentials)) {
      if (typeof password !== "string" || password.length < 4) continue;
...
export async function getXifadoState(): Promise<XifadoData> {
  ...
  if (configuredNames.length) {
    const seeded = structuredClone(parsed);
    let changed = false;
    for (const name of configuredNames) {
      const passwordHash = configured.credentials[name];
      const existingHash = await getCredentialHash(name);
      if (!existingHash) {
        await upsertCredentialHash(name, passwordHash);
        seeded.credentials[name] = passwordHash;
        changed = true;
      }
      if (!seeded.members[name]) {
        seeded.members[name] = configured.members[name];
        changed = true;
      }
    }
    if (changed) {
