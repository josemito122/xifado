...
O formulário de participante é o único ponto de entrada. O nome `mestre` leva à área mestre; qualquer outro nome é tratado como participante. Para participantes existentes, as credenciais precisam existir em `xifado_credentials`.

Se o banco atual ainda não tiver credenciais dos participantes, configure temporariamente no Vercel a variável `XIFADO_INITIAL_CREDENTIALS` como JSON, por exemplo `{"Expedito":"123456","Ana":"ABC123"}`. Faça uma implantação/leitura para semear os hashes e depois remova a variável. Os nomes podem ser digitados sem diferença de maiúsculas/minúsculas no login.
